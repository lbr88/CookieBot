...
class GardenManager {
    ...
    seedCalendar(garden, sector) {
        if (this._wantAscend || false)
            return 'bakerWheat'; // plant cheap before ascend (wantGardenSacrifice removed)
        if (sector === 0)
    ...
    findNextPlant(garden, sector) {
        ...
        for (let i = 3; i < PLANT_DEPENDENCIES.length; i++) {
            ...
            if (this.havePlant(garden, targetPlant))
                continue;
...
// Version
AutoPlay_AutoPlay.version = '2.052-5';
