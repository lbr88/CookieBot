...
class AchievementHandler {
    ...
    handleSmallAchievements() {
        ...
        if (!Game.Achievements['Cookie-dunker'].won && Game.milkProgress > 1 && Game.milkHd > 0.34) {
            if (this.backupHeight) {
                Game.LeftBackground.canvas.height = this.backupHeight;
                this.backupHeight = 0;
            }
            else {
                this.backupHeight = Game.LeftBackground.canvas.height;
        ...
        if (!Game.Achievements['No time like the present'].won &&
            ...
            if (!this.giftCode) {
                ...
                if (giftAmountEl && giftMessageEl && giftCodeEl && confirmBtn) {
                    ...
                    confirmBtn.click();
                    this.giftCode = giftCodeEl.value;
                    confirmBtn.click();
                    AutoPlay.info('Created present with code ' + this.giftCode);
                ...
                if (giftCodeEl && confirmBtn) {
                    giftCodeEl.value = String(this.giftCode);
                    this.giftCode = 0;
                    confirmBtn.click();
                }
            }
     ...
     */
    redeemPresent() {
        AutoPlay.info('Redeeming present with code ' + this.giftCode);
        if (this.giftCode) {
            Game.promptGiftRedeem();
            const giftCodeEl = l('giftCode');
            const confirmBtn = l('promptOption0');
...
class Dashboard {
            ...
            if (!isSavingActive) {
                ...
                if (AutoPlay.savingsStart !== undefined && AutoPlay.now) {
                    ...
                    if (elapsedTime < 0) {
                        ...
                        if (!Game.UpgradesById[52].bought)
                            missingUpgrades.push('Lucky day');
                        if (!Game.UpgradesById[53].bought)
                            missingUpgrades.push('Serendipity');
    ...
    getAchievementProgress() {
        ...
        if (achiev) {
            ...
            if (Game.Achievements["Hardcore"] && achiev.id === Game.Achievements["Hardcore"].id)
                isHardcore = true;
            if (Game.Achievements["Neverclick"] && achiev.id === Game.Achievements["Neverclick"].id)
                isNeverclick = true;
            if (Game.Achievements["True Neverclick"] && achiev.id === Game.Achievements["True Neverclick"].id)
                isTrueNeverclick = true;
        }
        ...
        if (!isSpecialAchievement && AutoPlay.workingOnSpecialAchievement && achiev) {
            ...
            if (activityText.indexOf('hardcore') !== -1)
                isHardcore = true;
            if (activityText.indexOf('true neverclick') !== -1)
                isTrueNeverclick = true;
            if (activityText.indexOf('neverclick') !== -1 && activityText.indexOf('true') === -1)
                isNeverclick = true;
        ...
        if (remaining > 0 && Game.cookiesPs > 0) {
            ...
            if (isHardcore && Game.UpgradesOwned !== 0)
                requirementsMet = false;
            if (isTrueNeverclick && Game.cookieClicks !== 0)
                requirementsMet = false;
            if (isNeverclick && Game.cookieClicks > 15)
                requirementsMet = false;
            if (requirementsMet) {
...
class GardenManager {
    ...
    planting(garden) {
        ...
        // Plant parent plants in each sector
        for (let sector = 0; sector < 4; sector++) {
            if (this.plantList[sector] === 0)
                continue; // No goal for this sector
            const plantGoal = PLANT_DEPENDENCIES[this.plantList[sector]][0];
            if (garden.plants[plantGoal].unlocked)
                continue; // Already have this plant
            this.plantsMissing = true;
            this.logActivity(`Sector ${sector}: Working on ${plantGoal}.`);
    ...
    findNextPlant(garden, sector) {
        ...
        for (let i = 0; i < PLANT_DEPENDENCIES.length; i++) {
            ...
            if (garden.plants[targetPlant].unlocked)
                continue;
            ...
            if (targetPlant === 'queenbeetLump') {
                ...
                if (centerPlant.key !== 'queenbeet')
                    continue;
                if (centerTile[1] < centerPlant.mature)
     ...
     * Original: AutoPlay.plantSector (lines 1274-1327)
     *
    ...
    plantSector(garden, sector) {
        ...
        const sectorX = sector % 2 ? 0 : 3;
        const sectorY = sector > 1 ? 0 : 3;
        // Special case: queenbeetLump just needs mature queenbeet in center (already checked)
        if (targetPlant === 'queenbeetLump') {
            // Let it mutate naturally - queenbeet has 0.1% chance per tick
            this.switchSoil(garden, sector, 'woodchips'); // Longer lifespan
            return;
        }
        // Special case: everdaisy needs both parents in middle column
        if (targetPlant === 'everdaisy') {
            this.switchSoil(garden, sector, 'woodchips'); // Longer lifespan
            // Plant parent1 and parent2 alternating in middle column
            for (let y = sectorY; y < sectorY + 3; y++) {
                const plantKey = (y - sectorY) % 2 === 0 ? parent1 : parent2;
                this.plantSeed(garden, plantKey, sectorX + 1, y);
            }
            return;
        }
        // Default planting pattern: checkerboard
        // Plant parent1 in left and right columns
        for (let y = sectorY; y < sectorY + 3; y++) {
            if (parent1 !== 'dummy') {
                this.plantSeed(garden, parent1, sectorX, y);
                this.plantSeed(garden, parent1, sectorX + 2, y);
            }
        }
        // Plant parent2 in middle column (if different from parent1)
        if (parent2 !== 'dummy' && parent2 !== parent1) {
            this.switchSoil(garden, sector, 'clay'); // Better mutation chance
            for (let y = sectorY; y < sectorY + 3; y++) {
                this.plantSeed(garden, parent2, sectorX + 1, y);
            }
        }
        else {
            // Same parent or no parent2: fill middle with parent1
            this.switchSoil(garden, sector, 'fertilizer'); // Faster growth
            for (let y = sectorY; y < sectorY + 3; y++) {
                if (parent1 !== 'dummy') {
                    this.plantSeed(garden, parent1, sectorX + 1, y);
                }
            }
        }
    }
...
// Version
AutoPlay_AutoPlay.version = '2.052.3';
