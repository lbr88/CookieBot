    ...
    updateModuleColumns() {
        ...
        try {
            ...
            if (miniModulesContainer) {
                // Show first 4 modules regardless of status
                const interestingModules = allModules.slice(0, 4);
                if (interestingModules.length === 0) {
                    ...
                    for (const { status } of interestingModules) {
                        ...
                        infoText = infoText.replace(/^(Working on:?|Buying|Upgrading|Waiting for)\s+/i, '');
                        ...
                        miniHtml += `<div style="position: relative; display: flex; align-items: center; gap: 6px; font-size: 10px; padding: 3px 8px; border-radius: 4px; background: rgba(255,255,255,0.05); overflow: hidden; min-width: 150px; border: 1px solid rgba(255,255,255,0.1);">`;
                        ...
                        // Content
                        miniHtml += `<span style="position: relative; color: ${color}; font-weight: bold;">${icon} ${status.module}</span>`;
...
// Version
AutoPlay_AutoPlay.version = '2.052-85';
